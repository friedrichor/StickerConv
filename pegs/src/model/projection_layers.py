...
class OutputProjector(nn.Module):
    ...
    def forward(self, x):
        x = self.fc1(x.float())  # [bs, 32, hidden_dim] = [b_s, 32, 512]
        x = self.tfm(x, self.query_embs.repeat(x.shape[0], 1, 1))  # [bs, num_clip_tokens, hidden_dim] = [b_s, 77, 512]
        output = self.fc2(x)  # [bs, num_clip_tokens, prompt_embeddings_dim] = [bs, 77, 768]
